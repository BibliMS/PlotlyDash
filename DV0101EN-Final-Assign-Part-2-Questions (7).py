...
def update_output_container(selected_statistics, input_year):
    if selected_statistics == 'Recession Period Statistics':
        ...
        R_chart3 = dcc.Graph(
              figure=px.pie(exp_rec,
                values='advertising_expenditure','vehicle_type'
                  names='Advertising_Expenditure', 'Vehicle Type'
        ...
        unemp_data= recession_data.groupby(['unemployment_rate', 'vehicle_sales'])['vehicle_type'].mean().reset_index()
        R_chart4 = dcc.Graph(
              figure=px.bar(
                  unemp_data,
                  x='Unemployment Rate',
        ...
        yas= data.groupby('Year')['Automobile Sales'].mean().reset_index()
        print(yas)
        Y_chart1 = dcc.Graph(figure=px.line(yas,
                                              x='Year',
                                              y='Sales',title="Yearly Average Automobile Sales"))

              # Total Monthly Automobile sales using line chart.
        Y_chart2 = dcc.Graph(figure=px.line(yearly_data,x='Month',
                      y='Sales',title="Monthly Automobile Sales for the year {}".format(input_year)))

              # Plot bar chart for average number of vehicles sold during the given year
        avr_vdata=data.groupby('vehicle_sales')['Year'].mean().reset_index()
        print(avr_vdata)
        ...
        Y_chart4 = dcc.Graph(figure=px.pie(exp_data,
                                            values='advertising_expenditure', 'vehicle_type'
                                            names='Advertising','Vehicle Type'
